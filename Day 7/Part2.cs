...
namespace Day_7
{
    ...
    {
        static List<string> validOperands = new List<string>() { "+", "*", "||" };
        ...
        static public long GetAnswer(List<Equation> input)
        {
            ...
            input.ForEach(eq =>
            {
                ...
                {
                    List<string> ops = opIndices.Select(opIndex => validOperands[opIndex]).ToList();
        ...
        static bool ExpressionEvalsTo(List<long> ints, List<string> operands, long target)
        {
            ...
            for (int i = 0; i + 1 < ints.Count; i++)
            {
                switch (operands[i])
                {
                    ...
                    case "||":
                        {
                            answer = long.Parse(answer.ToString() + ints[i + 1].ToString());
                            break;
                        }
